...
if not metro.empty and "id_paciente" in metro.columns and ciclo_col in metro.columns:
    df_ciclo = metro.copy()
    df_ciclo["presente"] = 1

    tabela_ciclos_hm = (
        df_ciclo.pivot_table(
            index=ciclo_col,
            columns="id_paciente",
            values="presente",
            aggfunc="max"
    ...
    sns.heatmap(tabela_ciclos_hm, cmap="Blues", cbar=True, ax=ax)

    ax.set_title("Presença de ciclos por paciente")
...
# =========================================================
# 📊 TABELA — Nº DE CICLOS POR PACIENTE (RESUMO)
# =========================================================
...
# =========================================================
# 🩸 HEATMAPS — GRID VISUAL (2 POR LINHA)
# =========================================================
...
for label, col in tox_cols:
    ...
    heatmaps.append((label, tabela))

# Renderização em grid
for i in range(0, len(heatmaps), 2):
    cols = st.columns(2)

    for j, (label, tabela) in enumerate(heatmaps[i:i+2]):
        with cols[j]:
...
# =========================================================
# 🔧 GARANTIR COLUNA DE CICLO (STREAMLIT)
# =========================================================
if not metro.empty:
    if ciclo_col not in metro.columns:
        # cria ciclo sequencial por paciente
        metro = metro.sort_values(["id_paciente"])
        metro["ciclo"] = (
            metro.groupby("id_paciente").cumcount() + 1
        )
        ciclo_col = "ciclo"
